...
// شعارا الاتحادية والرابطة — الملفان موضوعان في مجلد public بأسماء إنجليزية لتفادي مشاكل الترميز في Windows
// يُستعملان في: الشريط الجانبي، صفحة الدخول، العلامة المائية، لوحة التحكم، وبطاقة السباح (رأس البطاقة، خلفية البطاقة، وظهر البطاقة)
const FEDERATION_LOGO = '/federation-logo.jpg' // شعار الاتحادية الجزائرية للألعاب المائية
const LEAGUE_LOGO = '/league-logo.jpg' // شعار الرابطة الولائية للسباحة - المسيلة (LNM)
// أبقينا هذين الاسمين لتفادي تعديل بقية الكود، وكل واحد يشير الآن لشعاره الخاص
const CARD_LEAGUE_LOGO = LEAGUE_LOGO
const CARD_FEDERATION_LOGO = FEDERATION_LOGO

// خيارات صنف السباح المعتمدة اتحاديًا
const SWIMMER_CATEGORIES = ['مدارس', 'كتاكيت', 'براعم', 'أصاغر', 'أشبال']
// خيارات الزمرة الدموية
...
// وجه بطاقة السباح — يُستعمل في نافذة البطاقة الفردية وفي الطباعة الجماعية معًا
function SwimmerCardFront({ swimmer }) {
  return (
    <div
      className="swimmer-card card-front"
      style={{
        '--card-bg-left': `url("${CARD_FEDERATION_LOGO}")`,
        '--card-bg-right': `url("${CARD_LEAGUE_LOGO}")`,
      }}
    >
      <div className="card-header">
        <img src={CARD_FEDERATION_LOGO} alt="شعار الاتحادية" className="card-federation-logo" />
        <div className="card-header-text">
          <h3>الاتحاد الجزائري للألعاب المائية</h3>
          <span>الرابطة الولائية للألعاب المائية والسباحة - المسيلة</span>
        </div>
        <img src={CARD_LEAGUE_LOGO} alt="شعار الرابطة" className="card-league-logo" />
      </div>
      <div className="card-body">
        <div className="card-photo">
          <img src={swimmer.photo_url || '/default-avatar.png'} alt="السباح" />
          {swimmer.clubs?.logo_url && (
            <img src={swimmer.clubs.logo_url} alt="النادي" className="card-club-badge" />
          )}
        </div>
        <div className="card-details">
          <p><strong>الاسم:</strong> {swimmer.first_name}</p>
          <p><strong>اللقب:</strong> {swimmer.last_name}</p>
          <p><strong>تاريخ ومكان الميلاد:</strong> {formatCardDate(swimmer.birth_date)}{swimmer.birth_place ? ` بـ ${swimmer.birth_place}` : ''}</p>
          <p><strong>اسم الأب:</strong> {swimmer.father_name || '—'}</p>
          <p><strong>الزمرة الدموية:</strong> {swimmer.blood_type || '—'}</p>
          <p><strong>الصنف:</strong> {swimmer.category}</p>
          <p><strong>النادي:</strong> {swimmer.clubs?.name || 'بدون نادي'}</p>
...
// ظهر بطاقة السباح
function SwimmerCardBack({ swimmer }) {
  return (
    <div className="swimmer-card card-back">
      <div className="card-back-logos">
        <img
        ...
        />
        <img
          src={CARD_LEAGUE_LOGO}
          alt="شعار الرابطة الولائية للسباحة"
          className="card-back-logo"
          onError={e => { e.currentTarget.style.display = 'none' }}
        />
      </div>
      ...
      <div className="card-back-footer">
        ...
        <div className="card-back-number">
          ...
          <strong>{swimmer.registration_number || '—'}</strong>
        </div>
      </div>
    </div>
  )
...
function App() {
  ...
  const [bulkCardPrint, setBulkCardPrint] = useState(false)
  ...
  function openUserAdd() {
    if (!isFederationAdmin) return
    setUserForm({ full_name: '', email: '', password: '', role: 'club_admin', club_id: '' })
    setUserModal('add')
    setError('')
  ...
  async function saveUser(e) {
    ...
    if (userModal === 'add') {
      if (!userForm.email.trim()) return setError('البريد الإلكتروني مطلوب.')
      if (!userForm.password || userForm.password.length < 6) {
        return setError('كلمة المرور يجب ألا تقل عن 6 خانات.')
      }

      // نفس أسلوب إنشاء مدير النادي عند إضافة نادٍ جديد:
      // supabase.auth.signUp() يستبدل جلسة الدخول الحالية، لذا نحفظ جلسة المدير الحالي
      // ونعيدها بعد إنشاء الحساب الجديد حتى لا يتم تسجيل خروجه أو إدخاله بالحساب الجديد.
      ...
      try {
        ...
        if (authErr) { setError(authErr.message); signupFailed = true }
  ...
  function openSwimmerEdit(swimmer) {
    setSwimmerForm({
      ...
      father_name: swimmer.father_name || '',
      blood_type: swimmer.blood_type || '',
  ...
  async function saveSwimmer(e) {
    ...
    const payload = {
      ...
      father_name: swimmerForm.father_name.trim() || null,
      blood_type: swimmerForm.blood_type || null,
  ...
  return (
    <div className="app-shell" dir="rtl">
        ...
        {activePage === 'swimmers' && (
          <section>
            ...
            <div className="toolbar flex-gap">
              ...
              <button className="secondary" onClick={() => setBulkCardPrint(true)} disabled={!filteredSwimmers.length}>🖨️ طباعة بطاقات القائمة ({filteredSwimmers.length})</button>
            </div>
            <div className="table-card">
      ...
      {cardSwimmer && (
        <Modal title="بطاقة السباح الرسمية" onClose={() => setCardSwimmer(null)}>
          <div className="print-area">
            <SwimmerCardFront swimmer={cardSwimmer} />
            <SwimmerCardBack swimmer={cardSwimmer} />
          </div>
      ...
      {/* نافذة الطباعة الجماعية لبطاقات السباحين — 10 بطاقات في كل صفحة A4 */}
      {bulkCardPrint && (
        <Modal title={`طباعة بطاقات السباحين (${filteredSwimmers.length})`} onClose={() => setBulkCardPrint(false)} wide>
          <p className="bulk-print-hint">
            سيتم ترتيب وجوه البطاقات ثم أظهرها بنفس الترتيب، بمعدل 10 بطاقات في كل صفحة A4 (عمودان × 5 صفوف)،
            لتسهيل القص بعد الطباعة. لطباعة الظهر على وجه الورقة الآخر، اطبعوا الوجوه أولًا ثم الظهور بنفس الترتيب.
          </p>
          <div className="bulk-cards-section">
            <h4 className="bulk-cards-title">الوجوه</h4>
            <div className="cards-bulk-grid">
              {filteredSwimmers.map(s => <SwimmerCardFront key={`front-${s.id}`} swimmer={s} />)}
            </div>
          </div>
          <div className="bulk-cards-section bulk-cards-backs">
            <h4 className="bulk-cards-title">الظهور</h4>
            <div className="cards-bulk-grid">
              {filteredSwimmers.map(s => <SwimmerCardBack key={`back-${s.id}`} swimmer={s} />)}
            </div>
          </div>
          <div className="form-actions" style={{marginTop: '1.5rem'}}>
            <button className="primary" onClick={() => window.print()}>🖨️ طباعة كل البطاقات</button>
            <button type="button" onClick={() => setBulkCardPrint(false)}>إغلاق</button>
          </div>
        </Modal>
      )}
    </div>
...
function UsersPage({ users, search, setSearch, onAdd, onEdit }) {
  const roleLabel = (role) => role === 'federation_admin' ? 'مدير الاتحادية' : 'مدير نادي'

  return <section>
    <PageHead title="إدارة المستخدمين" button="إضافة مستخدم" onAdd={onAdd} count={users.length} />
    <div className="toolbar">
      <input
...
function UserModal({ form, setForm, clubs, onClose, onSave, editing, error }) {
  ...
  return <Modal title={editing ? 'تعديل المستخدم والصلاحيات' : 'إضافة مستخدم جديد'} onClose={onClose}>
    {error && <div className="alert">{error}</div>}
    <form onSubmit={onSave} className="form-grid">
      ...
      {!editing && (
        <>
          ...
          <label>كلمة المرور *
            <input type="password" required minLength={6} placeholder="6 خانات على الأقل" value={form.password} onChange={e => update('password', e.target.value)} />
          </label>
        </>
      )}
      <label>الدور *
      ...
      <div className="form-actions wide">
        ...
        <button className="primary">{editing ? 'حفظ الصلاحيات' : 'إنشاء الحساب'}</button>
      </div>
    </form>
  </Modal>
}
...
function Modal({ title, children, onClose, wide }) {
  return <div className="modal-backdrop"><div className={`modal${wide ? ' modal-wide' : ''}`}><div className="modal-head"><h2>{title}</h2><button onClick={onClose}>×</button></div>{children}</div></div>
}
...
function SwimmerModal({ form, setForm, setFile, photoFile, setPhotoFile, clubs, isClubAdmin, onClose, onSave, editing, error }) {
  ...
  return <Modal title={editing ? 'تعديل بيانات السباح' : 'إضافة سباح'} onClose={onClose}>
    ...
    <form onSubmit={onSave} className="form-grid two">
      ...
      <label>اسم الأب<input value={form.father_name} onChange={e=>update('father_name',e.target.value)} /></label>
      ...
      <label>الفئة *
        <select required value={form.category} onChange={e=>update('category',e.target.value)}>
          ...
          {SWIMMER_CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
          {form.category && !SWIMMER_CATEGORIES.includes(form.category) && <option value={form.category}>{form.category}</option>}
        </select>
      </label>
